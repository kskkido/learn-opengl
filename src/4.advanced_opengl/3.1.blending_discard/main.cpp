...
int main()
{
  ...
  std::filesystem::path grassTextureFilePath = staticFilePath / "grass.png";
  ...
  glBufferData(GL_ARRAY_BUFFER, grassVertices.size() * sizeof(float), &grassVertices[0], GL_STATIC_DRAW);
  ...
  unsigned int grassTexture = readTexture(grassTextureFilePath, false);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    for (glm::vec3& position : grassPositions)
    {
      ...
      grassModel = glm::translate(grassModel, position);
      glUniformMatrix4fv(glGetUniformLocation(textureShaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(grassModel));
      glDrawArrays(GL_TRIANGLES, 0, 6);
    }
