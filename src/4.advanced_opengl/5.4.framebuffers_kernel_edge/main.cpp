...
int main()
{
  ...
  std::filesystem::path quadVertexShaderFilePath = staticFilePath / "quad.vert";
  std::filesystem::path quadFragmentShaderFilePath = staticFilePath / "quad.frag";
  ...
  std::string quadVertexShaderSource = readFile(quadVertexShaderFilePath);
  unsigned int quadVertexShader = createShader(GL_VERTEX_SHADER, quadVertexShaderSource.c_str());
  std::string quadFragmentShaderSource = readFile(quadFragmentShaderFilePath);
  unsigned int quadFragmentShader = createShader(GL_FRAGMENT_SHADER, quadFragmentShaderSource.c_str());
  ...
  std::vector<unsigned int> quadShaders = {quadVertexShader, quadFragmentShader};
  ...
  glUniform1i(glGetUniformLocation(quadShaderProgram, "texture1"), 0);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    glm::mat4 projection = glm::perspective(glm::radians(state.fov), (float)bufferWidth / (float)bufferHeight, 0.1f, 100.f);
  ...
  for (unsigned int& shader : textureShaders)
  {
    glDeleteShader(shader);
  }
  for (unsigned int& shader : outlineShaders)
  {
    glDeleteShader(shader);
  }
