...
int main()
{
  ...
  glEnableVertexAttribArray(1);
  unsigned int cubeTexture = readTexture(marbleTextureFilePath, false);
  unsigned int planeTexture = readTexture(metalTextureFilePath, false);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    for (glm::vec3& position : cubePositions)
    {
      ...
      outlineModel = glm::scale(outlineModel, glm::vec3(1.1f, 1.1f, 1.1f));
      glUniformMatrix4fv(glGetUniformLocation(outlineShaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(outlineModel));
      glDrawArrays(GL_TRIANGLES, 0, 36);
    }
    glBindVertexArray(0);
    glStencilMask(0xFF);
