...
void handleFrameBufferChange(GLFWwindow* window, int width, int height)
{
...
int main()
{
  ...
  glViewport(0, 0, width, height);
  glfwSetFramebufferSizeCallback(window, handleFrameBufferChange);
  while (!glfwWindowShouldClose(window))
  {
    ...
    for (int i = 0; i < cubePositions.size(); ++i)
    {
      ...
      model = glm::rotate(model, (float)i * time * glm::radians(50.0f), glm::vec3(0.5f, 0.5f, 0.0f));
