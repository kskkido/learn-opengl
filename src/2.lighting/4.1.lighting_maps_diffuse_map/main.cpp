...
int main()
{
  ...
  GLenum format;
  if (containerTextureNrChannel == 1)
      format = GL_RED;
  else if (containerTextureNrChannel == 3)
      format = GL_RGB;
  else if (containerTextureNrChannel == 4)
      format = GL_RGBA;
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, containerTextureWidth, containerTextureHeight, 0, format, GL_UNSIGNED_BYTE, containerTextureData);
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    glUniform3fv(glGetUniformLocation(cubeShaderProgram, "material.specular"), 1, glm::value_ptr(glm::vec3(0.5f, 0.5f, 0.5f)));
    ...
    glUniform3fv(glGetUniformLocation(cubeShaderProgram, "light.position"), 1, glm::value_ptr(state.lightPosition));
